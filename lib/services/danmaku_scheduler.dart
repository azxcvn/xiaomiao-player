...
class DanmakuScheduler {
  ...
  /// 用一份**全量重算**后的条目集整体替换秒桶（P1-15 跨批次修正用）。
  ///
  /// 与 `reset() + feed()` 的区别：**不动代数、不动锚点**——
  /// - 保留代数 → 本 tick 已经派出的 stagger 延迟回调不作废（不会漏弹幕）；
  /// - 保留锚点 → 已发射过的秒桶不重放（否则整段历史弹幕会再倒一遍）。
  ///
  /// 发射是**只进不退**的：锚点之后的桶照常发射，锚点之前的桶即使被替换进来
  /// 也不会再发（与流式「先到先显、过时不补」语义一致）。
  void replaceAll(List<DanmakuEntry> entries) {
    _buckets.clear();
    feed(entries);
  }
