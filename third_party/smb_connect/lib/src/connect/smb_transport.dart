...
class SmbTransport {
  ...
  /// 串行化响应读取（避免并发在途请求时的重入竞态）。
  ///
  /// SocketReader2 的回调以「不等待」方式在每次数据到达时触发；当多个读请求同时在途
  ...
  ///
  /// 并发预读会有多个请求同时进入 doSend，而 Dart 的 Socket 写入端(_StreamSinkImpl)
  /// 不是重入安全的：一个协程在 flush() 期间 _isBound=true，另一个协程 add() 就会抛
  /// "StreamSink is bound to a stream"。原库靠全局锁隐式串行化了这一步；现在把锁去掉后，
  /// 必须显式用 future 链保证「写」仍然串行。响应等待仍走 messageId 并发的 sendrecv，
  /// 吞吐瓶颈在网络往返、不在写请求，因此不会影响提速效果。
  ...
  Future<T> sendrecv<T extends CommonServerMessageBlockResponse>(
    ...
    // 管线化改造：原来的全局互斥锁（sendrecvMutex）会串行化所有请求，导致同一时刻
    // 只有一个读请求在途，吞吐上限 = 单次读取块 ÷ 往返延迟（约 64KB/40ms ≈ 1.5MB/s）。
    // 这里移除该锁：SMB2 的 messageId 匹配（onListenReader 按 mid 分发到 responses）
    // 本就支持并发在途请求。为消除“响应先于挂载到达”的竞态，在发送前就分配 mid 并注册响应。
    final int key = ensureMid(request);
    ...
    for (var i = 0; i < 5; i++) {
      ...
      final Completer<CommonServerMessageBlockResponse> completer = Completer();
      responses[key] = (response: response, completer: completer);

      await doSend(request, params);

      try {
        final resp = await completer.future.timeout(waitResponseTimeout);
        if (config.debugPrint) {
          print("Recv $resp");
        }
        return resp as T;
  ...
  /// 分配（未分配的）messageId 并返回链头请求的 mid，用于 responses 的并发匹配。
  int ensureMid(CommonServerMessageBlockRequest request) {
  ...
  /// 把单条请求的「编码 + 写 socket + flush」串行送入 [_sendQueue]。
  Future<void> _enqueueSend(
      CommonServerMessageBlock smb, Uint8List buffer) {
    final run = _sendQueue.then((_) async {
      ...
      Encdec.encUint32BE(n & 0xFFFF, buffer, 0); // 4 byte session message header
